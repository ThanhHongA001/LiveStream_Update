package com.example.livestream_update.Ringme.LiveStream.socket.stomp;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import com.vtm.ringme.common.api.http.HttpsTrustManager;
import com.vtm.ringme.livestream.socket.stomp.provider.OkHttpConnectionProvider;
import com.vtm.ringme.livestream.socket.stomp.provider.WebSocketsConnectionProvider;
...
public class Stomp {
    ...
    public static StompClient over(@NonNull ConnectionProvider connectionProvider, String uri, @Nullable Map<String, String> connectHttpHeaders, @Nullable OkHttpClient okHttpClient) {
        ...
        OkHttpClient okHttp = HttpsTrustManager.getUnsafeOkHttpClient(60, false);
        if (connectionProvider == ConnectionProvider.OKHTTP) {
            return createStompClient(new OkHttpConnectionProvider(uri, connectHttpHeaders, okHttp));
        }

        throw new IllegalArgumentException("ConnectionProvider type not supported: " + connectionProvider.toString());
    }

    private static StompClient createStompClient(com.vtm.ringme.livestream.socket.stomp.provider.ConnectionProvider connectionProvider) {
