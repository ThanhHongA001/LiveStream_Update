 ...
 */
public class RSAEncryptLivestream {
    protected static final String ALGORITHM = "RSA";
    private static final String TAG = AESCrypt.class.getSimpleName();
    private static RSAEncryptLivestream mInstance;
    private SharedPreferences mPref;
    private PublicKey mPublicKey;

    private RSAEncryptLivestream(ApplicationController app) {
        try {
            mPref = app.getSharedPreferences(Constants.PREFERENCE.PREF_DIR_NAME, Context.MODE_PRIVATE);
            String key = mPref.getString(Constants.PREFERENCE.PREF_PUBLIC_RSA_KEY, UrlConfigHelper.RSA_LIVESTREAM_KEY);
    ...
    public static synchronized RSAEncryptLivestream getInStance(ApplicationController app) {
        if (mInstance == null) {
            mInstance = new RSAEncryptLivestream(app);
        }
        return mInstance;
    }
