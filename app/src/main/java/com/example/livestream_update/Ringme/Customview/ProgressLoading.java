...
public class ProgressLoading extends View {
    ...
    private void setAnimationEnabled() {
        ...
        try {
            if (currentApiVersion >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                animationValue = Settings.Global.getFloat(getContext().getContentResolver(), Settings.Global
                        .ANIMATOR_DURATION_SCALE, 1);
            } else {
                animationValue = Settings.System.getFloat(getContext().getContentResolver(), Settings.System
                        .ANIMATOR_DURATION_SCALE, 1);
            }
        ...
        shouldAnimate = (Float.compare(animationValue, 0) != 0);
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius = (int) a.getDimension(R.styleable.ProgressLoading_matProg_circleRadius, circleRadius);
        fillRadius = a.getBoolean(R.styleable.ProgressLoading_matProg_fillRadius, false);
        barWidth = (int) a.getDimension(R.styleable.ProgressLoading_matProg_barWidth, barWidth);
        rimWidth = (int) a.getDimension(R.styleable.ProgressLoading_matProg_rimWidth, rimWidth);
        float baseSpinSpeed = a.getFloat(R.styleable.ProgressLoading_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;
        barSpinCycleTime = a.getInt(R.styleable.ProgressLoading_matProg_barSpinCycleTime, (int) barSpinCycleTime);
        barColor = a.getColor(R.styleable.ProgressLoading_matProg_barColor, barColor);
        rimColor = a.getColor(R.styleable.ProgressLoading_matProg_rimColor, rimColor);
        linearProgress = a.getBoolean(R.styleable.ProgressLoading_matProg_linearProgress, false);
        if (a.getBoolean(R.styleable.ProgressLoading_matProg_progressIndeterminate, false)) {
            spin();
        }
        // Recycle
        a.recycle();
    ...
    public void setInstantProgress(float progress) {
        ...
        if (Float.compare(progress, mTargetProgress) == 0) {
            return;
        }
