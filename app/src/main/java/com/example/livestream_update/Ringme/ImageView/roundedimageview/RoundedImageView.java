...
package com.example.livestream_update.Ringme.ImageView.roundedimageview;
...
public class RoundedImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        aspect = a.getFloat(R.styleable.RoundedImageView_riv_aspect, -1);
    ...
    public void setCornerRadius(@Corner int corner, float radius) {
        if (Float.compare(mCornerRadii[corner], radius) == 0) {
            return;
        }
    ...
    public void setCornerRadius(float topLeft, float topRight, float bottomLeft, float bottomRight) {
        if (Float.compare(mCornerRadii[Corner.TOP_LEFT], topLeft) == 0
                && Float.compare(mCornerRadii[Corner.TOP_RIGHT], topRight) == 0
                && Float.compare(mCornerRadii[Corner.BOTTOM_RIGHT], bottomRight) == 0
                && Float.compare(mCornerRadii[Corner.BOTTOM_LEFT], bottomLeft) == 0) {
            return;
        }
    ...
    public void setBorderWidth(float width) {
        if (Float.compare(mBorderWidth, width) == 0) {
            return;
        }
