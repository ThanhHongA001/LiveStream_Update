 ...
 */
public class TagEmoticonManager {
    private static final String TAG = TagEmoticonManager.class.getSimpleName();
    private static final int EMO_CACHE_SIZE = 4 * 1024 * 1024;
    private static TagEmoticonManager instance = null;
    ...
    public static synchronized TagEmoticonManager getInstance(Context context) {
        if (instance == null) {
            instance = new TagEmoticonManager(context);
            Log.i(TAG, "instance == null");
        }
        return instance;
    }

    private TagEmoticonManager(Context ctx) {
        mApplication = (ApplicationController) ctx.getApplicationContext();
        mRes = mApplication.getResources();
        emoWeight = (int) (mRes.getDimensionPixelSize(R.dimen.kakoak_text_size_level_2_5) * emoWeightRatio);
        emoWeightBitmap = (int) (mRes.getDimensionPixelSize(R.dimen.kakoak_text_size_level_2_5) * 1.7);
        ///
